...
fit_poor_real_sim_archetype <- 
  brm(formula       = hedges_g | se(hedges_g_se) ~ 0 + Intercept + (1 | data_collection_site),
      prior         = c(prior(normal(0.20, 1.00), class = b),
                        prior(cauchy(0.00, 0.25), class = sd)),
      family        = gaussian(),
      data          = simulate_data(effect_size            = true_effect_size,
                                    k_sites                = k_sites,
...
fit_poor_null_sim_archetype <- 
  brm(formula       = hedges_g | se(hedges_g_se) ~ 0 + Intercept + (1 | data_collection_site),
      prior         = c(prior(normal(0.00, 1.00), class = b),
                        prior(cauchy(0.00, 0.25), class = sd)),
      family        = gaussian(),
      data          = simulate_data(effect_size            = true_effect_size,
                                    k_sites                = k_sites,
...
simulations$results %>% 
  ...
  ggplot(aes(x = rowname, y = estimate, ymin = lower, ymax = upper)) +
...
simulations$results %>% 
  ...
  mutate(rowname = as.numeric(as.character(rowname))) %>%
  ggplot(aes(x = rowname, y = estimate, ymin = lower, ymax = upper)) +
  facet_wrap(~ true_effect_string, ncol = 1) +
  annotate("rect",
           xmin  = -Inf, 
           xmax  = +Inf, 
           ymin  = rope_half_width*-1,
