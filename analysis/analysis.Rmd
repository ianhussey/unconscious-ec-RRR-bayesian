...
between_site_variation <- 0.10  # rounded down from meta of existing literature, given RRR's stardardized methods 
n_sim                  <- 2
...
if(file.exists("models/simulations_real.RData")){
  
  load("models/simulations_real.RData")
  
} else {
  
  # reset progress bar
  pb <- progress_estimated(n_sim*8)
  ...
  simulation_good_real <-
    tibble(seed = 1:n_sim) %>% 
    mutate(tidy = map(seed, simulation_iteration, 
  ...
  simulation_good_null <-
    tibble(seed = 1:n_sim) %>% 
    mutate(true_effect = true_effect_size,
           tidy = map(seed, simulation_iteration, 
                      effect_size            = true_effect_size,
...
simulations_real$results %>% 
  ...
  geom_hline(yintercept = true_effect_size, linetype = "dashed", color = "darkgreen") +
  geom_pointrange(fatten = 1/2) +
  labs(x = "Simulations iterations",
       y = expression(delta)) +
...
simulations_real$results %>% 
  mutate(null_test = ifelse(lower > 0, 1, 0),
         hdi_estimation = ifelse(lower < true_effect_size & upper > true_effect_size, 1, 0)) %>% 
...
I use ROPE = c(-0.16, 0.16) on the basis that a frequentist power analysis for meta analysis models demonstrated that our available sample size has 80% power to detect effect >= 0.16. I.e., agnostic to whether the effect is real or null, this is the estimation width we have 80% power to detect. 

This is slightly suboptimal as a Bayesian power analysis would be better, but simulating such multilevel data is non-trivial. There are working [examples](https://solomonkurz.netlify.com/post/bayesian-power-analysis-part-i/) for much simpler fixed effects only models however.
...
NB 66% and 95% HDIs. ROPE was $\delta$ = `r ROPE_cutoff*-1` to `r ROPE_cutoff`.
...
results_rope <- 
  bind_rows(
    mutate(bayestestR::equivalence_test(posterior_samples(fit_good_real)$b_Intercept, 
                                        ci = 0.95,
                                        range = c(ROPE_cutoff*-1, ROPE_cutoff),
                                        ci_method = "HDI"),
           prior = "Well-estimated real effect"),
...
## Olson & Fazio exclusion criterion

### Fit

```{r}

data_rrr_site_level_confirmatory <- 
  read.csv("../../unconscious-ec-RRR/data/processed/data_processed.csv") %>%
  mutate(data_collection_site = dplyr::recode(data_collection_site,
                                              "Balas and Sarzynnska" = "Balas",
                                              "Corneille and Mierop" = "Mierop",
                                              "Gast Richter and Benedict" = "Gast",
  ...
  filter(exclude_surveillance == FALSE & 
           simulated_data == FALSE) %>%
  rename(DV = sum_score_evaluation_CSpos_preferred) %>%
  filter(exclude_aware_olsen_and_fazio == FALSE) %>%
  dplyr::select(data_collection_site,
                DV,
                condition) %>%
  group_by(data_collection_site) %>%
...
fit_good_real_confirmatory <- update(fit_good_real, 
                                     newdata = data_rrr_site_level_confirmatory, 
                                     file = "models/fit_good_real_confirmatory")

fit_poor_real_confirmatory <- update(fit_poor_real, 
                                     newdata = data_rrr_site_level_confirmatory, 
                                     file = "models/fit_poor_real_confirmatory")

fit_good_null_confirmatory <- update(fit_good_null, 
                                     newdata = data_rrr_site_level_confirmatory, 
                                     file = "models/fit_good_null_confirmatory")

fit_poor_null_confirmatory <- update(fit_poor_null, 
                                     newdata = data_rrr_site_level_confirmatory, 
                                     file = "models/fit_poor_null_confirmatory")
...
p_results_confirmatory <- 
  bind_rows(
    mutate(spread_draws(fit_good_real_confirmatory, b_Intercept), prior = "Well-estimated real effect"), 
    mutate(spread_draws(fit_poor_real_confirmatory, b_Intercept), prior = "Poorly-estimated real effect"),
    mutate(spread_draws(fit_poor_null_confirmatory, b_Intercept), prior = "Poorly-estimated null effect"),
    mutate(spread_draws(fit_good_null_confirmatory, b_Intercept), prior = "Well-estimated null effect")
  ) %>%
  mutate(prior = fct_relevel(prior,
                             "Well-estimated null effect",
                             "Poorly-estimated null effect",
...
results_posterior_confirmatory <- 
  
  bind_rows(
    
    fit_good_real_confirmatory %>%
      posterior_summary(pars = "b_Intercept") %>%
      as.data.frame() %>%
      mutate(prior_certainty = "Well-estimated", 
             prior_effect = "real effect"),
    
    fit_poor_real_confirmatory %>%
      posterior_summary(pars = "b_Intercept") %>%
      as.data.frame() %>%
      mutate(prior_certainty = "Poorly-estimated", 
             prior_effect = "real effect"),
    
    fit_poor_null_confirmatory %>%
      posterior_summary(pars = "b_Intercept") %>%
      as.data.frame() %>%
      mutate(prior_certainty = "Poorly-estimated", 
             prior_effect = "null effect"),
    
    fit_good_null_confirmatory %>%
      posterior_summary(pars = "b_Intercept") %>%
      as.data.frame() %>%
      mutate(prior_certainty = "Well-estimated", 
             prior_effect = "null effect")
...
results_rope_confirmatory <- 
  bind_rows(
    mutate(bayestestR::equivalence_test(posterior_samples(fit_good_real_confirmatory)$b_Intercept, 
                                        ci = 0.95,
                                        range = c(ROPE_cutoff*-1, ROPE_cutoff),
                                        ci_method = "HDI"),
           prior = "Well-estimated real effect"),
    
    mutate(bayestestR::equivalence_test(posterior_samples(fit_poor_real_confirmatory)$b_Intercept, 
                                        ci = 0.95,
                                        range = c(ROPE_cutoff*-1, ROPE_cutoff),
                                        ci_method = "HDI"),
           prior = "Poorly-estimated real effect"),
    
    mutate(bayestestR::equivalence_test(posterior_samples(fit_poor_null_confirmatory)$b_Intercept, 
                                        ci = 0.95,
                                        range = c(ROPE_cutoff*-1, ROPE_cutoff),
                                        ci_method = "HDI"),
           prior = "Poorly-estimated null effect"),
    
    mutate(bayestestR::equivalence_test(posterior_samples(fit_good_null_confirmatory)$b_Intercept, 
                                        ci = 0.95,
                                        range = c(ROPE_cutoff*-1, ROPE_cutoff),
                                        ci_method = "HDI"),
           prior = "Well-estimated null effect")
...
# is the posterior effect size greater than zero?
results_p_greater_zero_confirmatory <- 
  bind_rows(
    mutate(p_direction(fit_good_real_confirmatory), prior = "Well-estimated real effect"),
    mutate(p_direction(fit_poor_real_confirmatory), prior = "Poorly-estimated real effect"),
    mutate(p_direction(fit_poor_null_confirmatory), prior = "Poorly-estimated null effect"),
    mutate(p_direction(fit_good_null_confirmatory), prior = "Well-estimated null effect")
  ) %>%
  mutate(p_greater_zero = 1 - pd) %>%
  dplyr::select(prior, p_greater_zero) %>%
  round_df(5)
...
# is the posterior effect size smaller than the published literature?
results_p_less_lit_confirmatory <- 
  bind_rows(
    mutate(hypothesis(fit_good_real_confirmatory, "Intercept < 0.20", alpha = 0.25)$hypothesis, 
           prior = "Well-estimated real effect"),
    mutate(hypothesis(fit_poor_real_confirmatory, "Intercept < 0.20", alpha = 0.25)$hypothesis, 
           prior = "Poorly-estimated real effect"),
    mutate(hypothesis(fit_poor_null_confirmatory, "Intercept < 0.20", alpha = 0.25)$hypothesis, 
           prior = "Poorly-estimated null effect"),
    mutate(hypothesis(fit_good_null_confirmatory, "Intercept < 0.20", alpha = 0.25)$hypothesis, 
           prior = "Well-estimated null effect")
  ) %>%
  mutate(p_less_lit = 1 - Post.Prob) %>%
  select(prior, p_less_lit) %>%
...
# is the posterior effect size smaller than the original study we replicate?
results_p_less_orig_confirmatory <- 
  bind_rows(
    mutate(hypothesis(fit_good_real_confirmatory, "Intercept < 0.27", alpha = 0.25)$hypothesis, 
           prior = "Well-estimated real effect"),
    mutate(hypothesis(fit_poor_real_confirmatory, "Intercept < 0.27", alpha = 0.25)$hypothesis, 
           prior = "Poorly-estimated real effect"),
    mutate(hypothesis(fit_poor_null_confirmatory, "Intercept < 0.27", alpha = 0.25)$hypothesis, 
           prior = "Poorly-estimated null effect"),
    mutate(hypothesis(fit_good_null_confirmatory, "Intercept < 0.27", alpha = 0.25)$hypothesis, 
           prior = "Well-estimated null effect")
  ) %>%
  mutate(p_less_orig = 1 - Post.Prob) %>%
  select(prior, p_less_orig) %>%
...
# combined
results_confirmatory <- results_posterior_confirmatory %>%
  left_join(results_rope_confirmatory, by = "prior") %>%
  left_join(results_p_greater_zero_confirmatory, by = "prior") %>%
  left_join(results_p_less_lit_confirmatory, by = "prior") %>%
  left_join(results_p_less_orig_confirmatory, by = "prior")
